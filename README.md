...
```bash
cargo run --bin=safenode-manager --features=local -- local run --build --clean --rewards-address <YOUR_ETHEREUM_ADDRESS> evm-custom --rpc-url <RPC_URL> --payment-token-address <TOKEN_ADDRESS> --data-payments-address <CONTRACT_ADDRESS>
```
...
```bash
cargo run --bin safe --features local -- files upload <path>
```
...
```bash
cargo run --bin safe --features local -- files download
```
...
```bash
cargo run --bin safe --features local -- folders init <dir-path>
```
...
```bash
cargo run --bin safe --features local -- folders status <dir-path>
```
...
```bash
cargo run --bin safe --features local -- folders sync <dir-path>
```
...
```bash
cargo run --bin safe --features local -- folders download <target dir path>
```
...
```
cargo run --bin safe --features local -- wallet send 2 [address]
```
...
```
cargo run --bin safe --features local -- wallet receive [transfer]
```
...
```
cargo run --bin safe --features local -- wallet verify [--genesis] [spend address]
```

All spends from genesis can be audited:

```
cargo run --bin safe --features local -- wallet audit
```
