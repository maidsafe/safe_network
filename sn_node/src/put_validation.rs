...
impl Node {
    ...
    pub(crate) async fn validate_and_store_chunk(
        ...
        if Self::chunk_validation(&chunk_with_payment, present_locally)?.is_none() {
    ...
    pub(crate) async fn validate_and_store_record(&self, mut record: Record) -> Result<(), Error> {
        ...
        match header.kind {
            RecordKind::Chunk => {
                ...
                let _new_data_present =
                    Self::chunk_validation(&chunk_with_payment, present_locally)?;
            }
            RecordKind::DbcSpend => {
    ...
    fn chunk_validation(
        ...
        }) = &chunk_with_payment.payment
        {
            ...
            if let Err(err) = validate_payment_proof(addr_name, reason_hash, audit_trail, path) {
                ...
                debug!("Chunk payment proof is valid!");
            }
        }
