...
use crate::subcommands::transfer::data::chunk_manager::ChunkManager;
...
pub(crate) async fn upload_files_with_iter(
    ...
    let mut chunks_to_upload = if chunk_manager.is_chunks_empty() {
        ...
        if failed_chunks.is_empty() {
            ...
            print_filename_and_hex(chunk_manager);
    ...
    let upload_result = match files_upload.upload_chunks(chunks_to_upload).await {
        Ok(()) => {Ok(())}
        Err(ClientError::Transfers(WalletError::Transfer(sn_transfers::Error::NotEnoughBalance(
            available,
            required,
        )))) => {
...
fn print_filename_and_hex(chunk_manager: ChunkManager) {
