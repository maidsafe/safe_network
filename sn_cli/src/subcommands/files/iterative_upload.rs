        ...
        while let Some(event) = upload_event_rx.recv().await {
            match event {
                FileUploadEvent::Uploaded(addr) => {
                    ...
                    if let Err(err) = ChunkManager::mark_completed_async(
                        chunk_manager.clone(),
                        std::iter::once(*addr.xorname()),
                    ) {
...
fn terminate_with_error(
    ...
    if upload_terminated_with_error {
        ...
        msg_check_incomplete_files(chunk_manager.clone());
...
fn msg_check_incomplete_files(chunk_manager: ChunkManager) {
