...
build-release-artifacts arch nightly="false":
  ...
  if [[ $arch == arm* || $arch == armv7* || $arch == aarch64* ]]; then
    ...
    cross build --release --features network-contacts --target $arch --bin ant $nightly_feature
    cross build --release --features network-contacts --target $arch --bin antnode $nightly_feature
    ...
    cargo build --release --features network-contacts --target $arch --bin ant $nightly_feature
    cargo build --release --features network-contacts --target $arch --bin antnode $nightly_feature
