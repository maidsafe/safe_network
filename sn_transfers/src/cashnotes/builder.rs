...
pub struct TransactionBuilder {
    inputs: Vec<Input>,
    outputs: Vec<Output>,
    input_details: BTreeMap<
        UniquePubkey,
        (
            Option<DerivedSecretKey>,
            DerivationIndex,
            BTreeSet<UniquePubkey>,
        ),
    >,
...
impl TransactionBuilder {
    ...
    pub fn add_input(
        ...
        parent_spends: BTreeSet<UniquePubkey>,
    ) -> Self {
        self.input_details.insert(
            *input.unique_pubkey(),
            (derived_key, derivation_index, parent_spends),
        );
    ...
    pub fn add_inputs(
        ...
        for (input, derived_key, derivation_index, parent_spends) in inputs.into_iter() {
            self = self.add_input(input, derived_key, derivation_index, parent_spends);
        }
        self
    }
    ...
    pub fn build_unsigned_transfer(
        ...
        for input in &self.inputs {
            if let Some((_, derivation_index, parent_spends)) =
                self.input_details.get(&input.unique_pubkey)
            {
                let spend = Spend {
                    unique_pubkey: *input.unique_pubkey(),
