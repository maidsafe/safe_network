...
async fn test_folder_basics() -> Result<()> {
    ...
    let (file_entry_hash, file_meta_xorname, file_metadata) =
        folders_api.add_file("file.txt".into(), file_chunk.clone())?;
    assert_eq!(
        file_metadata,
        Metadata {
    ...
    let (subdir_entry_hash, subdir_meta_xorname, subdir_metadata) =
        folders_api.add_folder("subdir".into(), address_subdir)?;
    assert_eq!(
        subdir_metadata,
        Metadata {
...
async fn test_folder_remove_replace_entries() -> Result<()> {
    ...
    let (file1_entry_hash, _, _) = folders_api.add_file("file1.txt".into(), file1_chunk.clone())?;
    let (file2_entry_hash, file2_meta_xorname, file2_metadata) =
        folders_api.add_file("file2.txt".into(), file2_chunk.clone())?;
    ...
    let (file3_entry_hash, file3_meta_xorname, file3_metadata) =
        folders_api.replace_file(file2_entry_hash, "file3.txt".into(), file3_chunk)?;
    assert!(!folders_api.contains(&file2_entry_hash));
    ...
    let (file4_entry_hash, file4_meta_xorname, file4_metadata) =
        folders_api.add_file("file4.txt".into(), file4_chunk)?;
...
async fn test_folder_retrieve() -> Result<()> {
    ...
    let (file1_entry_hash, file1_meta_xorname, file1_metadata) =
        folder.add_file("file1.txt".into(), file1_chunk.clone())?;
    let (subfolder_entry_hash, subfolder_meta_xorname, subfolder_metadata) =
        folder.add_folder("subfolder".into(), *subfolder.address())?;

    let file2_chunk = random_file_chunk();
    let (file2_entry_hash, file2_meta_xorname, file2_metadata) =
        subfolder.add_file("file2.txt".into(), file2_chunk.clone())?;
    ...
    folder.sync(false, None).await?;
    subfolder.sync(false, None).await?;
...
async fn test_folder_merge_changes() -> Result<()> {
    ...
    let (file_a1_entry_hash, file_a1_meta_xorname, file_a1_metadata) =
        folder_a.add_file("fileA1.txt".into(), file_a1_chunk.clone())?;
    let (subfolder_a_entry_hash, subfolder_a_meta_xorname, subfolder_a_metadata) =
        folder_a.add_folder("subfolderA".into(), *subfolder_a.address())?;
    let (file_a2_entry_hash, file_a2_meta_xorname, file_a2_metadata) =
        subfolder_a.add_file("fileA2.txt".into(), file_a2_chunk.clone())?;
    ...
    let (subfolder_b_entry_hash, subfolder_b_meta_xorname, subfolder_b_metadata) =
        folder_b.add_folder("subfolderB".into(), *subfolder_b.address())?;
    let (file_b2_entry_hash, file_b2_meta_xorname, file_b2_metadata) =
        subfolder_b.add_file("fileB2.txt".into(), file_b2_chunk.clone())?;
    ...
    folder_a.sync(false, None).await?;
    subfolder_a.sync(false, None).await?;
    folder_b.sync(false, None).await?;
    subfolder_b.sync(false, None).await?;
    folder_a.sync(false, None).await?;
    subfolder_a.sync(false, None).await?;
