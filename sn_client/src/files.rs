...
use crate::chunk_operator::BuildChunkOperator;
...
impl FilesApi {
    ...
    pub async fn upload_test_bytes(&self, bytes: Bytes, verify: bool) -> Result<NetworkAddress> {
        ...
        let chop = BuildChunkOperator::new().build_with_data_map_in_chunks(&file_path, &chunk_path);

        for (_chunk_name, chunk_path) in chop.chunk_vec {
            let chunk = Chunk::new(Bytes::from(fs::read(chunk_path)?));
            self.get_local_payment_and_upload_chunk(chunk, verify, None)
                .await?;
        }

        Ok(NetworkAddress::ChunkAddress(chop.head_chunk_address))
    }
}
