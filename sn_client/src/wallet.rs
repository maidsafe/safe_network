...
impl WalletClient {
    ...
    /// Clear up ContentPaymentsMap and store
    pub fn clear_payments_and_store(&mut self) {
        self.wallet.clear_content_payment_map();
        match self.wallet.lock() {
            Ok(exclusive) => {
                let _ = self.wallet.store(exclusive);
            }
            Err(err) => error!("Failed to fetch the exclusive locker {err:?}"),
        }
    }
}
