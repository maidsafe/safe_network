...
impl Files {
    ...
    pub async fn pay_and_upload_bytes_test(
        ...
        while !failed_chunks.is_empty() {
            println!(
                "Repaying for {:?} chunks, so far paid {storage_cost}|{royalties_fees}",
                failed_chunks.len()
            );
            ...
            println!(
                "NEW cost for {:?} chunks: {new_storage_cost}|{new_royalties_fees}",
                failed_chunks.len()
            );
            ...
            println!("NEW TOTAL cost: {storage_cost}|{royalties_fees}");
