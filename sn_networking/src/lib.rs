...
use libp2p::{
    identity::Keypair,
    kad::{
        KBucketDistance as Distance, KBucketKey, Kademlia, KademliaConfig, KademliaStoreInserts,
...
const IDENTIFY_VERSION_STR: &str = concat!("safe/node/", env!("CARGO_PKG_VERSION"));
...
impl SwarmDriver {
    ...
    pub fn new(
        ...
        let (network, events_receiver, mut swarm_driver) = Self::with(
            ...
            IDENTIFY_VERSION_STR.to_string(),
        )?;
