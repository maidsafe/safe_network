...
//                                    if let Some(Protocol::P2p(remote_peer_id)) = addr.iter().last() {
//                                        if remote_peer_id == peer_id.clone() {
...
//                                    }
                            ...
                            && info.agent_version.starts_with(IDENTIFY_AGENT_STR)
                        {
                            ...
                            // If remote is a relay client remove any associated addresses already stored in kad's routing table, or else add them and identify those running specific protocols
