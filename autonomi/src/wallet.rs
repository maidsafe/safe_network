...
use sn_evm::wallet::{
    create_a_evm_wallet, create_file_with_keys, get_client_wallet_dir_path, get_gas_token_details,
    get_random_private_key, prompt_the_user_for_password, wallet_encryption_status,
    wallet_encryption_storage, ENCRYPTED_MAIN_SECRET_KEY_FILENAME,
};
...
pub fn get_private_key_from_wallet(
    key: u32,
        ...
        Some(value) => {
            ...
            if value.0 == "unprotected" {
                ...
                if let Ok(file_data) = file_contents {
                    let private_key = String::from_utf8(file_data).map_err(|_| {
                        EvmError::OperationError("Not able to convert file contents".to_string())
                    })?;
            ...
            if value.0 == "passw-protected" {
                wallet_directory.push(ENCRYPTED_MAIN_SECRET_KEY_FILENAME);
                ...
                if let Ok(file_data) = encrypted_bytes {
                    let encrypted_private_key = String::from_utf8(file_data).map_err(|_| {
                        EvmError::OperationError("Not able to convert file contents".to_string())
                    })?;
                    ...
                    match password {
                        ...
                        None => {
                            println!(
                                "The provided Password is not proper, can not proceed further"
                            );
                        }
                    }
                }
    ...
    // TODO: Why not return the error immediately?
    let return_value: Option<String> = None;

    return_value.ok_or(EvmError::OperationError(
        "Not able to get the private key".to_string(),
    ))
}

pub fn get_numbered_files(dir: PathBuf) -> std::io::Result<HashMap<u32, (String, String)>> {
    let mut file_map: HashMap<u32, (String, String)> = HashMap::new(); // Create a new HashMap to store the files
...
pub fn prompt_for_key() -> Result<u32, EvmError> {
    ...
    std::io::stdout()
        .flush()
        .map_err(|_| EvmError::OperationError("Not able to flush out stdio".to_string()))?;

    let mut input = String::new();

    std::io::stdin()
        .read_line(&mut input)
        .map_err(|_| EvmError::OperationError("Not able to read the line contents".to_string()))?;

    let input_value: u32 = input
        .trim()
        .parse()
        .map_err(|_| EvmError::OperationError("Not able to read the line contents".to_string()))?;
    Ok(input_value)
